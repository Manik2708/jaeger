...
func TestIndexCreateIfNotExist(t *testing.T) {
	...
	for _, test := range indexExistsTests {
	...
	tests := []struct {
		...
		{
			name: "alias existing error",
			createIndexReturnErr: client.ResponseError{
				Err:        errors.New("x"),
		...
		},
		{
			...
			createIndexContainsError: "invalid character",
		},
		{
			name: "success",
		},
